...
void myMouseFunc(int button, int state, int x, int y)
{
	...
	if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN && (x >= 1418 && x <= 1498) && (y >= 773 && y <= 829)) {
		pages = 1;
		resetpage2();
	}
	
	if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN && (x >= 34 && x <= 128) && (y >= 770 && y <= 829)) {
			exit(0);
		}
...
void display() {
	if (pages == 1) {
		...
		char h[] = "HADES";
		glColor3f(1, 1, 1);
		bigdrawString(910, 350, 0, z);
		bigdrawString(1405, 260, 0,p);
		bigdrawString(380, 260, 0,h );
		...
		glColor3f(1, 1, 0);
		char s3[] = "MOUNT OLYMPUS";
		bigdrawString(800, 975, 0, s3);
		
		bubble(450, 375, 1, 1);
		glColor3f(1, 1, 0);
		drawcirc(-175, -270, 0, 40);
		glColor3f(0, 0, 0);
		char s[] = "CLICK ME";
		char s1[] = "EXIT";
		drawString(960, 820, 0, s);
		drawString(85,75, 0, s1);
		glColor3f(1, 1, 0);
